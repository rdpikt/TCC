...
(function () {
  ...
  document.addEventListener('click', function(event) {
    // Verifica se o elemento clicado ou um de seus pais é um botão de Entrar/Sair
    let button = event.target.closest('.btn-entrar, .btn-sair');

    if (button) {
        event.preventDefault();

        const comunidadeId = button.getAttribute('data-comunidade-id');
        const acao = button.classList.contains('btn-entrar') ? 'entrar' : 'sair';
        
        // Exemplo: Executar requisição AJAX
        // fazerRequisicaoAjax(comunidadeId, acao).then(response => {
        //     // Lógica de sucesso: trocar a classe e o texto do botão
        //     // ...
        // });
    }
});